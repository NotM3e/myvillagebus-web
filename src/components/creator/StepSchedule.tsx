    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {PRESETS.map((preset) => {
            const isActive = preset.days.every(d => data.days.includes(d)) && 
          ...
          <div className="mt-4 space-y-3">
            ...
            {validCourses.length > 0 && (
              <div className="p-3 rounded-xl bg-[var(--md-sys-color-primary-container)]">
                <div className="flex items-center gap-2 mb-2">
                  <AccessTimeIcon sx={{ fontSize: 18, color: 'var(--md-sys-color-on-primary-container)' }} />
                  <span className="md-label-large text-[var(--md-sys-color-on-primary-container)]">
                    Rozpoznano {validCourses.length} {validCourses.length === 1 ? 'kurs' : validCourses.length < 5 ? 'kursy' : 'kursów'}
                  </span>
                </div>
            ...
            {invalidCourses.length > 0 && (
              <div className="p-3 rounded-xl bg-[var(--md-sys-color-error-container)]">
                <div className="flex items-center gap-2 mb-2">
                  <WarningAmberIcon sx={{ fontSize: 18, color: 'var(--md-sys-color-on-error-container)' }} />
      ...
      {validCourses.length > 0 && data.days.length > 0 && (
        <div className="p-4 rounded-xl bg-[var(--md-sys-color-secondary-container)]">
          ...
          <ul className="space-y-1 md-body-medium text-[var(--md-sys-color-on-secondary-container)]">
            <li>• Kierunek: <strong>{data.direction}</strong></li>
            <li>• Dni: <strong>{data.days.join(', ')}</strong></li>
            <li>• Kursów: <strong>{validCourses.length}</strong></li>
            <li>• Przystanków: <strong>{data.stops.length}</strong></li>
            {data.excludesHolidays && (
